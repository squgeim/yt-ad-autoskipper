(
  // skippable ads will be skipped instantly
  // the old features will also work just in case utube makes changes
  // unskippable ads must be muted in a future version
  function () {
    var classList = [
    ...
    // we also need to check when ad is visible
    var bannerList = [
      'ytp-ad-text'
    ]
    var timeoutId;
     ...
     */
    // for unskippable ads, will use in future.. cant seem to get it to work yet becuz 
    // unskippable ads are rare and its hard to test
    function muteAd() {
      existingButtons(muteList).map(btn => {
        if (btn.title === "Unmute (m)") {
          triggerClick(btn);
        }
      })
    }
    ...
    function checkAndClickButtons() {
      ...
      existingButtons([...bannerList, ...classList]).forEach(element => {
        ...
        //  click the skip ad button before the 'skip Ad' shows
        // also click when the 'skip ad' button is only clickable after 5 seconds
        if (!isElementVisible(element)) {
          // trigger button
          /* looping over classList again, we want to push classlist buttons 
          including when ad banner[video] is visible 
          you can add other video ad identifiers if u like
          */
          existingButtons(classList).map(btn => {
